*CMZ :          29/09/2023  16.43.11  by  Michael Scheer
...
*-- Author :    Michael Scheer   12/12/2016
      subroutine clcmag_to_radia_mshradia
...
*KEND.
     ...
     &        "]," // trim(adjustl(c32z)) // "))"
            ...
            print*,"*** Error in clcmag_to_radia_mshradia: mode",
     &        matmaps(2,imat), " not yet implemented for REC material ",imat
            print*,""
            !stop "--- UNDUMAG aborted ---"
          endif
        else
          print*,"*** Error in clcmag_to_radia_mshradia: Unknown material or mode", imat,
     &      matmaps(:,imat)
          print*,""
          !stop "--- UNDUMAG aborted ---"
        endif
...
c                call util_string_append(cline,'],',nfirst,nlast)
     ...
     &          trim(adjustl(t_magcopy(imp)%cnam)) // ")"
              ...
              if (t_magcopy(imp)%cnam.eq.chforcemag) then
                ...
                write(lunrad,'(a)')"nForce = " // trim(adjustl(c32))
                write(lunrad,'(a)')
                print*,"*** Warning in clcmag_to_radia_mshradia: Force calculations for cylinders not yet tested..."
              endif
      ...
      if (istat.ne.0) then
        ...
        write(lun6,*)"*** Error in clcmag_to_radia_mshradia: File undumag_proc_msh_radia.py not found ***"
