...
11      x=xmapmin+(ix-1)*dx
     ...
     &      ix,x,y,z
      ...
      if (irecover.ne.0) then
        write(lun6,*)
        write(lun6,*)"*** Warning in undumag_end:  On-axis field calculations with ",irecover,
     &    " recovered errors ***"
        write(lun6,*)
c        do ix=1,ixon
...
c Calculate field map{
      ...
      if (kmapnohead.eq.0) then
        ...
        if (kmapmode.eq.0) then
          ...
          write(lun,'(a)')"* imoth imag mat ityp matmod x/mm y/mm z/mm Bx/T By/T Bz/T B/T Hx/T "
     &      //"Hy/T Hz/T H/T Mx/T My/T Mz/T M/T BxDip/T ByDip/T BzDip/T ifail kfail cmag cmoth"
        else
          write(lun,'(a)') '* ' // trim(ctitle)
          write(lun,'(a)')"* " // trim(chtime)
...
c              write(lun6,*)"field:",ix,iy,iz
     ...
     &            sngl(xx),sngl(yy),sngl(zz),sngl(bx),sngl(by),sngl(bz),ifail,kkfail
        ...
        if (irecover.ne.0) then
          write(lun6,*)
          write(lun6,*)"*** Warning in undumag_end:  Field calculations with ",irecover,
     &      " recovered errors ***"
          write(lun6,*)
        endif
     ...
     &        sngl(x),sngl(y),sngl(z),sngl(bx),sngl(by),sngl(bz)
        ...
        if (irecover.ne.0) then
          write(lun6,*)
          write(lun6,*)"*** Warning in undumag_end:  Field calculations inside magnets with ",
     &      irecover, " recovered errors ***"
          write(lun6,*)
        endif
      ...
      if (knomagmap.eq.0) then
        ...
        do kmag=1,nrec
          ...
          if (newclc.ne.0) then
            ...
            if (kbrn.eq.0) then
              write(lun6,*)
              write(lun6,*)"*** Warning in undumag_end: Could not find REC material in list of "
     &          //"Br ***"
     ...
     &        sngl(x),sngl(y),sngl(z),sngl(bx),sngl(by),sngl(bz)
        ...
        if (irecover.ne.0) then
          write(lun6,*)
          write(lun6,*)"*** Warning in undumag_end:  Field calculations inside iron with ",
     &      irecover, " recovered errors ***"
          write(lun6,*)
        endif
...
c      i_debug=1
     ...
     &          -2.0d0/y2z2*pz
              ...
              write(lun6,*)"*** Error in undumag_end: Integration of dipole fields fails, since "
     &          //"distance to dipole is zero for dipole",kmag," ***"
            endif
          enddo
        endif !(idipoles.ne.0) then
